// unlock.js - the full-screen intro announcement (from Matt to the recipient). Played
// once by the hub on first activation, and replayable from inside the Challenge Area.
// Voice: warm, silly, personal (see memory challenge-copy-voice); NOT secret-agent LARP.
// Self-contained; injects challenge.css.
...
export function playUnlock(onDone) {
  ensureChallengeCss();
  const name = esc((loadProfile() || {}).name || 'you');   // runtime, so no literal name in source
  ...
  el.innerHTML = `
    ...
    <div class="ch-unlock-card">
      ...
      <p class="ch-unlock-kicker">hi, you</p>
      <h1 class="ch-unlock-title">Operation<br>${name}</h1>
      <p class="ch-unlock-sub">You found it. Of course you found it. I made you a ridiculous little game, and there's a real prize at the end.</p>
      <p class="ch-unlock-fine">(It's a lot. I know it's a lot. Humor me.)</p>
      <button type="button" class="ch-btn ch-btn-go" data-role="accept">Okay, show me</button>
    </div>`;
