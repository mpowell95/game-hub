...
| `js/net.js` | multiplayer room layer (`rooms/<CODE>`, lockstep move log, heartbeat, recovery, SW-version match on join) used by Chinchón, Escoba, Tic Tac Toe, Mancala, Filler, Dots and Boxes and Poolv2 |
...
### The seventh consumer: Poolv2 (physics build, not the MP roadmap doc)

Full write-up: `poolv2/CLAUDE.md`. `js/net.js` was NOT touched. Built alongside Poolv2's initial
...
- **A "move" is shot PARAMETERS, not a discrete game move.** Every reference game above transmits
  something from its own finite move vocabulary (a mark, a pit index, an edge). Poolv2's engine is
...
- **The shooter still applies its own shot immediately** (same "don't make the mover wait on a
  ...
  identical params on delivery and verifies a state hash (`poolv2/js/hash.js`) — structurally the
...
- **Status: unverified beyond inline reasoning.** No `test-mp-lockstep.mjs` block exists for this
  ...
  against a `FakeRoom` harness. Flagged honestly in `poolv2/CLAUDE.md` rather than claimed proven.
