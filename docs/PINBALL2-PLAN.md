...
| Goes through objects | Step-then-push-out. Small steps make it rare, not impossible | Swept collision: solve the exact time of first contact, move the ball there, bounce, repeat. Plus a net for the one case that misses: a ball that is ALREADY inside a collider, which both impact tests treat as not their surface | 76392 balls fired hard from every reachable point, 0 left the machine. **Added 2026-09-10 after Matt found a ball leaving the table in about a minute**: the three checks that shipped first all missed it |
