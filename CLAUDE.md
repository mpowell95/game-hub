...
- **Pass 4 (done):** second deck **`naipe-fino`** — a custom gold-medallion deck built
  from a supplied coin image (oros = the coin at pip positions; copas/espadas/bastos =
  generated embossed gold suit medallions; figures = medallion + gold crest + name
  banner; navy/gold back). A **deck picker** in setup (mirrors the avatar picker:
  `open-deck`/`pick-deck`/`close-deck`) calls `setDeck()` + `preloadDeck()`, persists
  `deck` in `chinchon-settings`, and shows a fanned preview per deck. Both decks are
  precached in `sw.js`. Deck assets were generated by a scratch `sharp` script, not in-repo.
- **Later polish (not built):** more decks, one-undo affordance, sound.
