...
| `test-dotsboxes-stats.mjs`, `test-filler-stats.mjs`, `test-mancala-stats.mjs`, `test-poolv2-stats.mjs` | (2026-07-30/31) the **recorded-exactly-once** family (needs `jsdom`; skips cleanly without it). Each drives its game's REAL `ui.js` and asserts a session's worth of games each record once — no more, no fewer. Two opposite failures, one mechanism: the first three replay "play three games in a row" after a `_statsCommitted` flag that was never reset silently dropped every game after the first; Poolv2's replays an MP hash-mismatch recovery landing on an already-finished game, which recorded it a SECOND time (and as a loss, if the peer's authoritative state disagreed on the winner). **Add a case here for any game whose recorder gains a guard, and always test both directions** — a guard with no per-game reset and a reset with no guard are both real bugs this repo has shipped |
