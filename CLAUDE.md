...
- **Pass 4 (done):** a second, additional deck **`anita`** — an Española *skin* that
  swaps the **Oros pips** (ranks 1–9) for a supplied gold "Ana" coin and adds a **custom
  card back** (Ana's photo, background-removed, on an original green/gold symmetric
  design), leaving every other card (Oros figures, all Copas/Espadas/Bastos) as standard
  Española art. It ships just those 10 WebP files and inherits the rest via the
  `base`/`own` override (see "Card decks"); the default deck stays `baraja-libre`. A
  **deck picker** in setup (mirrors the avatar picker: `open-deck`/`pick-deck`/`close-deck`)
  calls `setDeck()` + `preloadDeck()`, persists `deck` in `chinchon-settings`, and shows a
  fanned preview per deck. `sw.js` precaches baraja-libre's 49 files + anita's 9 pips + back.
  The custom art was generated by scratch `sharp` scripts (not in-repo) from `coin_clean.png`
  (pips) and `anita.jpg` (back).
- **Roadmap (not built):** themed Anita suits (copas = stein, bastos = golf driver,
  ...
  win/lose screens — all pending art. Plus: one-undo affordance, sound.
