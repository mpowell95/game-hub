...
| `test-dotsboxes-stats.mjs`, `test-filler-stats.mjs`, `test-mancala-stats.mjs`, `test-pool-stats.mjs`, `test-poolv2-stats.mjs`, `test-yahtzee-stats.mjs` | (2026-07-30/31) the **recorded-exactly-once** family (needs `jsdom`; skips cleanly without it). Each drives its game's REAL `ui.js` and asserts a session's worth of games each record once — no more, no fewer. One mechanism, both failure directions: dots-boxes/filler/mancala replay "play three games in a row" after a `_statsCommitted` flag that was never reset silently dropped every game after the first; poolv2 replays an MP recovery landing on an already-finished game, which recorded it a SECOND time (and as a loss, if the peer's authoritative state disagreed on the winner); pool and yahtzee replay the opposite — an MP resync/mismatch on the WINNING move, after which the match was recorded on neither device. **Add a case here for any game whose recorder gains a guard, always test both directions** (a guard with no per-game reset and a reset with no guard are both real bugs this repo has shipped) **and always check what the game's RESYNC path does with an already-finished game**, not just its move path — that one line is what three separate games got wrong |
| `test-bd-drain.mjs` | (2026-07-31) THE LAW rule 6 for `gamehub.bd.pendingStats.v1`, the queue holding Monopoly Deal plays that missed `window.__ghStats` at game-end. Proves a failed store write LEAVES the queue intact (it used to be deleted before the write that absorbed it, so one quota failure lost the plays from both places) and that a queue surviving several failed loads is still applied exactly once. Node-only, no deps |
