...
| `test-visual.mjs` | (2026-08-08) the only suite that LOOKS at a game. Drives every one in a real Chromium at 393x852 in light/dark/reduced-motion and fails on: nothing painted, the body scrolling sideways, a JS error on mount, or an animation too brief to follow (`MOTION` probes). Writes a contact sheet to `.visual-out/` - **open it.** SKIPs without playwright-core/Chromium. Written after Battleship's cannon took four rounds of Matt's time: **`VISUAL-PROCESS.md` and `reference/` are the process it belongs to, and a session doing visual work must read them first.** |
