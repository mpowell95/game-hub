...
| `sweep-mover.mjs` | (2026-08-25) reachability on a machine with a MOVING PART - Skeeball's HOT SHOT: RUNAWAY, whose top-row 100 basket slides across the face all game. Every other machine's reachability is a 2-D question (power x aim) and `skeeball/js/test.js`'s 41x21 grid answers it; a moving hole makes the same swipe land somewhere different depending on where the basket was at release, so that grid measures ONE arbitrary frozen phase and reports the moving hole as unreachable or as trivial at random. This walks power x aim x PHASE over a full period and reports captures per hole, the jam rate against a 1% budget, the slowest settle, and which phases score the mover at all. `node sweep-mover.mjs runaway`; re-run after any change to that face, its amplitude, its period or its materials |
