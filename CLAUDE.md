...
| `js/bug-report.js` | (2026-08-11) "Report a bug": gathers the device/browser/PWA/network/SW picture PLUS the whole Device Details payload, sends it to `bugReports/` (screenshots to `bugReportShots/`), and keeps an offline outbox that retries itself |
| `js/bug-report-ui.js` | the report form + Matt's admin inbox. The repo's FIRST consumer of `css/ui.css`'s `.gh-*` primitives |
| `js/error-log.js` | ring buffer of the last 20 uncaught JS errors (`gamehub.errorlog.v1`), installed by `hub.js` at load so a bug report carries what actually threw |
| `js/announce.js` | one-time launcher announcements: the entries, the seen-list (`gamehub.announce.v1`), and the pure "does this device still owe one" decision. Each entry carries an `until` date and retires itself |
...
| `test-bug-report.mjs` | (2026-08-11) headless tests for the pure halves of Report a bug: the screenshot budget, the description clamp, the environment summary, and the announcement's show-once/expire-by-itself decision (including the shipped announcement's own dates and EN/ES completeness, the silent failure here being an announcement that never appears). The DOM and Firebase halves are NOT covered and the suite header says so. |
| `read-bug-reports.mjs` | (2026-08-11) Matt-only, the terminal-side view of the in-app inbox: lists `bugReports/` newest first, prints one in full, and is the only way to get the screenshots onto disk - `node read-bug-reports.mjs [--open] [<id> [--shots [dir]]] [--json]` |
