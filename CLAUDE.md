...
- **Highlight-sets uses one ring color, not one per meld.** The `cc-meld-c0..c5` classes used to each carry a different hue (including both green and red) with no non-color differentiator - a direct THE LAW rule 9 violation. All melds now ring in the single shared `#ffce3a` accent, with a small numbered badge (`data-meld-num`, a card's own `::after`) as the actual differentiator between simultaneous melds.
