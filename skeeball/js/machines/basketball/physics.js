 ...
 *  body - the ball - among 200 static ones (126 cup segments over 9 holes, plus the cabinet, lane, hump,
 ...
 *  physics. This walks the ball's own pairs instead: 200 tests, same answer.
 *
 ...
 *
 *  HOT SHOT ONLY, per the HARD RULE in skeeball/CLAUDE.md. Each machine that has this has its own
...
function substep(st) {
  ...
  if (f.v > 0 && f.v < G.boardLen && f.h < G.ballR * 1.9) {
    for (const id of Object.keys(G.holes)) {
      ...
      // ball is relative to IT. See machine.js worldToFaceIn.
