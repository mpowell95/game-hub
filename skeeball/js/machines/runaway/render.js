// skeeball/js/machines/runaway/render.js - HOT SHOT: RUNAWAY's OWN renderer. This file serves
...
export class Renderer {
  constructor(canvas, board) {
    ...
    this._wallMarks = [];         // transient scuffs where the ball struck the back wall
  ...
  _buildMachine() {
    ...
    // reads as a glitch sliding across a shelf; with it, it reads as a machine part on a rail,
    // which is what it is. Drawn AFTER the baskets so it never z-fights the mouth it runs under.
    if (G.mover) this._moverTrack();
   ...
   *  end, standing up off tread 3.
   *
   *  GUARD: THEY STAND UP, AND THAT IS THE WHOLE REASON THEY EXIST. This started as a rail: a
   *  painted groove lying flat on tread 3 spanning the travel, with these two posts capping it.
   *  The groove was drawn, correctly placed, and COMPLETELY INVISIBLE - not a contrast problem
   *  (it was retried in near-black and in the lit accent) but a geometry one. The camera stands
   *  behind the ball, so tread 3 is the furthest and most foreshortened surface on the machine
   *  and is occluded outright by the middle row's riser. Nothing lying FLAT on that shelf can be
   *  seen from where the game is played. The posts read because they have height and clear the
   *  riser's edge. The groove was removed rather than dimmed: a mesh no player can ever see is
   *  dead geometry, and this comment is here so it does not get "restored" as an oversight.
   *
   *  GUARD: NO PHYSICS BODY, and it must stay that way. They sit OUTSIDE the travel, clear of
   *  the basket, and a real solid there would put a lip on the strip of tread a missed top-row
   *  ball has to roll back over.
   *
   *  If a future machine puts a mover on a NEARER shelf, a flat rail would read there and is
   *  worth having - the finding above is about this camera and this shelf, not about rails. */
  _moverTrack() {
    const G = this.G;
    const m = G.mover;
    ...
    for (const side of [-1, 1]) {
      const stop = new THREE.Mesh(
        this._track(new THREE.BoxGeometry(railW * 0.22, railW * 0.62, railW * 1.0)),
        this._mat({
          color: steel, roughness: 0.5, metalness: 0.5,
          emissive: steel, emissiveIntensity: 0.5,
        }),
      );
      this._onFace(stop, side * (m.amp + H.r * 0.78), H.v, railW * 0.3);
      this.scene.add(stop);
    }
  }
  ...
  /** A rounded rect path on a 2D context. Carried over from BRICK CITY's renderer, which needed
   *  it for its brick sign; here it draws the marquee's second-line plate. */
  ...
  /** The sign over the backboard.
   *
   *  GUARD: THE NAME IS MEASURED AND SHRUNK TO FIT, NEVER DRAWN AT A FIXED SIZE. HOT SHOT's copy
   *  of this function drew `board.name` at a hardcoded 104px, which is fine for a name as short
   *  as its own and CLIPS ANY LONGER ONE against the 1024px canvas - "HOT SHOT: RUNAWAY" lost a
   *  letter off each end, and this machine is where that first showed. Same rule the rest of the
   *  repo uses for text that has to fit a box (tic-tac-toe/CLAUDE.md, and the 11px floor in
   *  docs/BUILDING-A-GAME.md Part 0): measure the real rendered width and size down until it fits.
   *
   *  A "PREFIX: NAME" name is stacked the way BRICK CITY's sign stacks its own - the family name
   *  large, the machine's own name on a plate under it - because two fitted lines read far better
   *  at this size than one line squeezed to half height. A name with no colon stays one line, so
   *  this function is still correct if it is ever copied to a machine called something short. */
  _paintMarquee() {
    const W = 1024;
    const H = 192;
    ...
    const PAD = 48;                       // clear of the panel's own edges at both ends
    const fits = (text, px) => {
      x.font = `700 ${px}px Georgia, serif`;
      return x.measureText(text).width <= W - PAD * 2;
    };
    /** The largest size at or under `max` that fits, never below `min` (a name long enough to
     *  need smaller than that is a naming problem, not a rendering one - and it still draws). */
    const fitSize = (text, max, min = 28) => {
      let px = max;
      while (px > min && !fits(text, px)) px -= 2;
      return px;
    };
    ...
    const i = this.board.name.indexOf(':');
    if (i > 0) {
      const top = this.board.name.slice(0, i).trim();
      const bottom = this.board.name.slice(i + 1).trim();
      draw(top, fitSize(top, 92), H * 0.34);
      // The plate under it: a lozenge in the marquee text colour with the machine's own name
      // knocked out of it, which is how BRICK CITY's sign carries the same second line.
      const bPx = fitSize(bottom, 58);
      x.font = `700 ${bPx}px Georgia, serif`;
      const bw = x.measureText(bottom).width;
      const py = H * 0.73;
      const ph = bPx * 1.34;
      x.fillStyle = this.look.marqueeText;
      this._roundRect(x, (W - bw) / 2 - 26, py - ph / 2, bw + 52, ph, 6);
      x.fill();
      x.fillStyle = this.look.marquee;
      draw(bottom, bPx, py + 2);
    } else {
      draw(this.board.name, fitSize(this.board.name, 104), H / 2 + 4);
    }
  ...
  render(game, dt) {
    // MOVER: the basket's pose for this frame, from the SAME pure function physics.js drives the
    // collision bodies with, evaluated at the SAME rack clock (game.js's machineT). That shared
    // input is the whole reason the drawn basket and the one the ball hits cannot disagree - and
    ...
    if (live.length || used !== this._shadowBalls || movers !== this._shadowMovers
      || this._celebrateT > 0 || this._wallMarks.length > 0) {
      this.renderer.shadowMap.needsUpdate = true;
    }
  ...
  // PORTED FROM BRICK CITY (2026-09-03), where it was built and tuned. Same cabinet, same
  // 10.9 cm ball and the same near-black back wall, so the size and the pale colour carry over
  // unchanged; the wall's own plane is read from machine.js, so nothing here is hard-coded to a
  // machine. See skeeball/MACHINE-BRICKCITY.md for the three passes it took to get right.
  /** THE SCUFF THE BALL LEAVES ON THE BACK WALL - a soft dark mark, lying IN the wall, at the
   *  point it was struck.
   *
   *  Matt asked for this because a bounce off the back of the machine had no visual at all
   *  ("It's impossible to tell where on the back wall a ball that's overthrown bounces off").
   *  The FIRST version of it was wrong in three ways he then had to point out - a bright gold
   *  ring, drawn on the side rails as well, and BILLBOARDED to face the player. All three are
   *  corrected here, and the reasons are worth keeping:
   *
   *  1. IT LIES IN THE SURFACE, it does not turn to face the camera. Matt: "to have the ring
   *     appear face on to the user as a perfect circle when it hits a wall that's almost
   *     perpendicular to the player is crazy. That's not the norm for stuff like this." He is
   *     right, and the norm has a name: a DECAL - a texture projected onto the surface, which
   *     foreshortens with it (three.js ships DecalGeometry for exactly this). This wall is flat,
   *     so it needs no projection machinery: a quad in the wall's own plane IS the decal, and it
   *     costs one draw call. polygonOffset is the standard way to keep it off the wall's z rather
   *     than shoving it bodily forward.
   *  2. IT IS A SCUFF, NOT A SIGN. Matt: "a big, solid, neon ring that's super distracting... it's
   *     gotta be tampered down significantly... maybe it's just a semi dark scuff mark that fades
   *     away after a couple seconds." So: a soft dark radial smudge, no rim, no glow, about one
   *     ball across, gone in two seconds.
   *  3. THE BACK WALL ONLY (see physics.js's 'wall' event, which no longer fires for a rail), and
   *     only the FIRST wall contact of a throw. A ball that rattles between the wall and the
   *     baskets leaves one mark, not six.
   *
   *  GUARD: the texture and the quad are built ONCE and shared. A canvas texture per impact is
   *  the leak popupAt was caught making (see the .map note in render()); only the per-mark
   *  material is disposable, and it is disposed when the mark expires. */
  _scuffTexture() {
  ...
  dispose() {
    ...
    for (const m of this._wallMarks) { this.scene.remove(m); disposeMat(m.material); }
    this._popups = [];
    this._particles = [];
    this._wallMarks = [];
