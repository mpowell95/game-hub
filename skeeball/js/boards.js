...
// THE ORDER OF THIS ARRAY IS THE ORDER OF THE GALLERY, and it must follow the unlock chain.
// Nothing derives an unlock from it - every machine names its own parent in `unlock.board` -
// so the two can drift, and they did: the 2026-08-27 swap that put RUNAWAY ahead of POPONGO
// re-pointed both entries' `unlock` and left this array alone, so the carousel went on
// showing POPONGO fourth and RUNAWAY last for a week. Matt, 2026-09-02: "make popongo the
// last game. Runaway should come before it." Reordered here, and `SK_MACHINES` in
// js/game-stats-ui.js (the hub's own copy of the machine list) is kept in the same order.
// When the chain changes, move the entry too.
export const BOARDS = [
  {
    id: 'classic',
...

  {
    ...
    geom: {
      ...
      minSpeed: 2.60,
      // THE POWER DIAL, 6.60 -> 6.20 (2026-09-05, Matt: "The power thing we just updated is
      // better. implement this", after playing the admin-only tuning twin that carried it).
      //
      // At 6.60 the dial was 12 bands with a FOLD-BACK at the top: dead 0-14%, the -10 to 42%,
      // the 20 to 58%, the 50 to 80%, and then the last 12% went BACK DOWN to the 20 - throw
      // harder, score less, with no way to read that off the machine. At 6.20 it is a clean,
      // monotonic four - dead 0-20%, then the penalty row, the 20 and the 50 at about a quarter
      // of the dial each - and the top of the dial stays at the top. Measured on 51 straight
      // throws; on the 861-throw power x aim grid every basket is still reachable, the corner
      // 100s keep their cells (8 + 9 -> 8 + 7), the 50 gains (7 -> 12) and the grid total goes
      // UP, 760 -> 800.
      //
      // GUARD: THE CURVE'S SHAPE IS NOT THE LEVER, and that was measured before this landed, not
      // assumed. Power is spent as ENERGY (physics.js interpolates v^2), and the obvious "make it
      // linear" is linear in SPEED, which is worse on every count - the dead zone at the bottom
      // grows 16% -> 24% and throws scoring nothing go 10/51 -> 15/51. Nor is the answer to raise
      // minSpeed and delete the dead zone: that swings the grid from 760 to -550, because every
      // weak throw that used to die harmlessly in the trough lands in a PENALTY basket instead.
      // The dead zone is doing work. Only the top of the range wanted trimming.
      maxSpeed: 6.20,
