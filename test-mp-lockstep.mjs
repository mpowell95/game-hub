...
// (Chinchón M2b, Escoba M1, Tic Tac Toe phase 1, Mancala phase 2, Dots and Boxes phase 2). No
// Firebase, no DOM: an in-file FakeRoom
...
//   Dots and Boxes (dots-boxes/js/ui.js) - the same _mp* family plus _localSeat/_remoteSeat,
//     _isLegal, _mpNewState, _mpEncodeMove/_mpDecodeMove, _mpAfterLocalMove,
//     _mpTryDeliverNextMove/_mpApplyNextEntry (ASYNC, same reasoning as Mancala - each remote
//     EDGE gets a brief paced delay so a chain capture reads as a sequence, not a snap),
//     _mpOnDivergence, _mpSnapshot, _mpApplyRecovery, _mpStartNextGame/_mpApplyRoundRecord,
//     _mpAwaitNextGame, _mpAfterGameEnd, _mpRoomCallback/_mpOnRoomUpdate, _mpSaveSnapshot/
//     _mpLoadSave, _tryRestoreMP, and the MP branch of _afterStateChange. MOVE GRANULARITY: one
//     lockstep move per drawn EDGE (matches game.js's applyMove()), never a whole chain capture
//     batched into one move - the design decision this file's own header block explains was made
//     web-session-only and needs a real-network latency check (a long chain on Large is many
//     rapid edges inside one real turn). Also named rather than line-numbered, same reasoning.
