...
## How-to-play screens — the repo-wide pattern

The pattern itself (worked out here, 2026-07-21) moved to `docs/BUILDING-A-GAME.md`, "Part 2 —
Screen patterns," so it's discoverable by any game's session, not just this one. **The reference
implementation still lives here**: `tic-tac-toe/js/ui.js`'s `openHelp()`, whose diagram (nine
board outlines, one showing its own mini-grid with a marked cell, and an arrow curving to the
board that cell sends the opponent to) is the concrete example the pattern doc describes in the
abstract.
