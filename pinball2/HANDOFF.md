...
### The ramp tool (2026-09-11)

Matt: *"why can't i add ribbons?"* The answer was that nobody had built a way. Every other part is
two or three numbers you can drag; a ramp's shape IS a path, and three rules have to hold along it
that freehand dragging breaks instantly.

**Tap Ramp, tap points on the table, tap Done.** The curve, the point spacing and the height profile
are generated by **`buildRamp` in `table.js` - the same function the shipped table calls**, never a
copy in the editor. Two copies of a curve agree until somebody fixes one of them.

- **The control points are STORED on the shape (`ctrl`).** That is what makes a ramp re-editable.
  Re-fitting control points out of the hundred-odd generated ones is guesswork, and a ramp you
  cannot reshape is a ramp you delete and lay again. `toJSON`/`fromJSON` pass shapes through
  verbatim, so it needed no codec change.
- **A ramp's handles ARE its control points.** Drag one and the whole curve regenerates. Move the
  ramp by dragging its body. **This changed what the two end dots do** on a ramp that has control
  points; a ramp built before the tool existed has none and keeps the old end-dot behaviour (move
  the whole thing), because inventing control points for it would be a guess.
- **`moveShape`, `rotateShape` and `scaleShape` all carry `ctrl` alongside `pts`.** Let them fall
  out of step and the next drag of a control handle snaps the ramp back to where the control points
  still thought it was.
- **`rampPoints` resamples by DISTANCE, not by a fixed number of steps per control segment.** A path
  a person taps has segments of wildly different lengths, and a fixed step count makes the long ones
  coarse, which is exactly where a kink appears. This replaced the old 8-steps-per-segment code;
  TEST BOX's own ramp was re-measured through `rampProbe` afterwards and still passes.
- **The crest is clamped away from either end.** A path that only ever heads down the table has its
  apex at its first point, which would make the whole ramp one long fall and divide by zero besides.
  Such a path gets its crest in the middle.
- **Under two control points produces no path at all**, rather than a shape made of NaN.
- The live preview draws the REAL generated curve in the selection accent, dashed, at the lane's
  real width. Drawing the taps alone tells you nothing about the ramp you are making, because the
  curve bulges where you did not put a point and that bulge is what a ball rides. The accent rather
  than the ramp blue, because the first version was indistinguishable from a ramp already there.

**`rampProbe` moved into the Check tab as the `Ramps` button.** It was node-only, which was fine
while every ramp in existence was written in code and checked once. The moment a person can lay one
by hand, a check that lives in a terminal is a check that never runs.
