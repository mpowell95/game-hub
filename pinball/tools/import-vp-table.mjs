...
const out = [
  ...
  ' *  nanosPerFrame * targetTimeRatio every frame, so this table runs at ' + Number(src.targetTimeRatio) + 'x REAL TIME.',
  ' *  Run it at 1x and every shot, drop and bounce is ' + Number(src.targetTimeRatio) + 'x too slow: the free fall down this',
  ' *  field goes from a real machine-like ' + (Math.sqrt(2 * FH / Number(src.gravity)) / Number(src.targetTimeRatio)).toFixed(2) + 's to ' + Math.sqrt(2 * FH / Number(src.gravity)).toFixed(2) + 's. That is exactly what',
  ' *  the 1x build felt like, and Matt described it in three words. */',
  'export const TIME_RATIO = ' + Number(src.targetTimeRatio) + ';',
  '',
