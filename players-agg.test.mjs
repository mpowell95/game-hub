...
// Three cases: a `hand` merge by the SMALLER timestamp (the earliest choice stands), a history row
// that exists on only one device (must still appear on the combined record), and a higher trophy
// recorded on the second device (Math.max, never a sum).
{
  const all = {
    d1: rec({ playerId: 'BB111', name: 'Bud' }, {
      baseball: {
        ...
        bb: {
          ...
          hits: 10, doubles: 1, triples: 0, homers: 2, atBats: 30, runs: 8, walksDrawn: 3,
          runsAllowed: 5, hitsAllowed: 9, walksIssued: 2, inningsPitchedOuts: 27,
          strikeoutsBatting: 4, strikeoutsPitched: 6, shutouts: 0, walkoffWins: 1, extraInningGames: 1,
          bestRunsGame: 4, bestStrikeoutsPitchedGame: 5,
    ...
    d2: rec({ playerId: 'bb111', name: 'Bud' }, {
      baseball: {
        ...
        bb: {
          ...
          hits: 5, doubles: 0, triples: 1, homers: 0, atBats: 15, runs: 3, walksDrawn: 1,
          runsAllowed: 2, hitsAllowed: 4, walksIssued: 1, inningsPitchedOuts: 12,
          strikeoutsBatting: 2, strikeoutsPitched: 3, shutouts: 1, walkoffWins: 0, extraInningGames: 0,
          bestRunsGame: 2, bestStrikeoutsPitchedGame: 8,
  ...
  eq('baseball: integer counters summed across devices', [bb.careersStarted, bb.careersFinished, bb.seasons, bb.forfeits, bb.wsTitles, bb.perfectSeasons],
    [2, 1, 5, 1, 1, 1]);
  eq('baseball: batting/pitching counters summed', [bb.hits, bb.doubles, bb.triples, bb.homers, bb.atBats, bb.runs, bb.walksDrawn],
    [15, 1, 1, 2, 45, 11, 4]);
